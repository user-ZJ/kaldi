...
template<class Holder>  class SequentialTableReaderBufferImpl:
      public SequentialTableReaderImplBase<Holder> {
 public:
  typedef typename Holder::T T;

  SequentialTableReaderBufferImpl(): state_(kUninitialized) { }

  virtual bool Open(const std::string &rspecifier) {return false; }
  virtual bool Open(vec_ss *rs) {
    if (state_ != kUninitialized) {
      if (!Close()) {
        KALDI_ERR << "Error closing previous input";
      }
    }
    KALDI_ASSERT(rs == NULL);
    index = 0;
    key_ = "";
    rs_ = rs;
    vec_len = rs_->size();
    state_ = kFileStart;
    if (vec_len == 0) state_ = kError;
    if (state_ == kError) {
      KALDI_WARN << "Error beginning to read buffer file";
  ...
  virtual void Next() {
    ...
    while (1) {
      key_ = rs_->at(index).getString();
      bool isRead = holder_.Read(*(rs_->at(index).getStrSS()));
      if (isRead) {
        state_ = kHaveObject;
        index++;
        return;
      }else {
        del_vec_.push_back(index);
        KALDI_WARN << "Object read failed,reading buffer key_ is " << key_ << " index is" << index;
        state_ = kError;
      }
      index++;
      if (index >= vec_len) {
  ...
  virtual std::string Key() {
    switch (state_) {
      case kHaveObject:
        break;
      default:
        KALDI_ERR << "Key() called on buffer reader object at the wrong time.";
      }
      return key_;
  }
  ...
  T &Value() {
    switch (state_) {
      case kHaveObject:
        break;
      default:
        KALDI_ERR << "Value() called on bufferReader object at the wrong time.";
    }
  ...
  virtual bool Close() {
    if (!this->IsOpen())
      KALDI_ERR << "Close() called on BufferReader twice or otherwise wrongly.";
    int32 status = 0;
  ...
  virtual ~SequentialTableReaderBufferImpl() {
...
template<class Holder>  class SequentialTableReaderScriptImpl:
 ...
 public:
  ...
  virtual bool Open(vec_ss *rs) {return false;}
...
//add by zack start 
template<class Holder>
SequentialTableReader<Holder>::SequentialTableReader(vec_ss* rs): impl_(NULL) {
  if (rs != NULL && !Open(rs))
    KALDI_ERR << "Error constructing TableReader" ;
}
//add by zack end

template<class Holder>
bool SequentialTableReader<Holder>::Open(vec_ss *rs) {
	if (IsOpen())
		if (!Close())
	...
	impl_ = new SequentialTableReaderBufferImpl<Holder>();
...
template<class Holder>
class TableWriterBufferImpl : public TableWriterImplBase<Holder> {
  public:
    typedef typename Holder::T T;
    virtual bool Open(const std::string &wspecifier) { return false; }
    virtual bool Open(vec_ss *vs) {
      switch (state_) {
        case kUninitialized:
          break;
        case kWriteError:
    ...
    virtual bool IsOpen() const {
      switch (state_) {
        ...
        case kWriteError:
          return true;
        default:
          KALDI_ERR << "IsOpen() called on TableWrite in invalid state.";
      }
      return false;
    }
    ...
    virtual bool Write(const std::string &key, const T &value) {
      ...
      if (!IsToken(key))
        ...
        vs_->at(index).setString(key);
      if (!Holder::Write(*(vs_->at(index).getStrSS()), true, value)) {
        KALDI_WARN << "Write failure to buffer";
        state_ = kWriteError;
        return false;
      }
    ...
    virtual bool Close() {
      ...
      if (state_ == kWriteError) {
        KALDI_WARN << "closeing writer in error state.";
        state_ = kUninitialized;
        return false;
      }
      state_ = kUninitialized;
    ...
    TableWriterBufferImpl() :state_(kUninitialized) {}
	
    virtual ~TableWriterBufferImpl() {
      if (!IsOpen())
        return;
      else if (!Close())
...
class TableWriterArchiveImpl: public TableWriterImplBase<Holder> {
 public:
  ...
  virtual bool Open(vec_ss *vs) {return false;}
...
//add by zack start
template<class Holder>
TableWriter<Holder>::TableWriter(vec_ss* vs): impl_(NULL) {
  if (vs != NULL && !Open(vs))
    KALDI_ERR << "Failed to open table for writing with buffer: "
              << ": errno (in case it's relevant) is: " << strerror(errno);
}
//add by zack end
...
template<class Holder>
bool TableWriter<Holder>::Open(vec_ss *vs) {
	if (IsOpen()) {
		if (!Close())  // call Close() yourself to suppress this exception.
			KALDI_ERR << "Failed to close previously open writer.";
	}
	KALDI_ASSERT(impl_ == NULL);
	impl_ = new TableWriterBufferImpl<Holder>();
...
template<class Holder> class RandomAccessTableReaderImplBase {
 public:
  ...
  //add by zack start 
  virtual bool Open(map_ss* rs,const std::string &range) = 0;
  //add by zack end
...
template<class Holder>
class RandomAccessTableReaderBufferImpl :
		public RandomAccessTableReaderImplBase<Holder> {
  public:
    typedef typename Holder::T T;
    RandomAccessTableReaderBufferImpl() :state_(kUninitialized) {}
    virtual bool Open(const std::string &rspecifier) { return false; }
    virtual bool Open(map_ss *rs, const std::string& range) {
      switch (state_) {
        case kNotHaveObject:
        case kHaveObject:
        case kHaveRange:
          KALDI_ERR << "Opening already ipen RandomAccessTableReader,call close first.";
        case kUninitialized:
        case kNotReadScript:
          break;
      }
    ...
    virtual bool IsOpen() const {
      return (state_ == kNotHaveObject || state_ == kHaveObject || state_ == kHaveRange);
    }
    ...
    virtual bool HasKey(const std::string &key) {
      return HasKeyInternal(key, false);
    }
    virtual const T& Value(const std::string &key) {
      if (!HasKeyInternal(key, true))
        KALDI_ERR << "Could not get item for key " << key << ",[to ignore this,add the p,(permissive) option to the rspecifier.";
      if (state_ == kHaveObject) {
        state_ = kNotHaveObject;
        return holder_.Value();
      }else {
    ...
    virtual ~RandomAccessTableReaderBufferImpl() {}
  private:
      ...
      if (!LookupKey(key)) {
        ...
        if (!preload) {
          ...
          if (state_ == kNotHaveObject) {
            std::istream & ifs = *((*rs_)[key]);
            if (holder_.Read(ifs)) {
              state_ = kHaveObject;
            }else {
              KALDI_WARN << "Error reading object from stream.";
              return false;
            }
          }
          if (range_.empty()) {
          ...
          if (range_holder_.ExtractRange(holder_, range_)) {
            ...
            KALDI_WARN << "Failed to load object from buffer [" << range_ << "]";
            return false;
          }
        }
      }
    ...
    bool LookupKey(const std::string &key) {
      map_ss::iterator it = rs_->find(key);
      if (it != rs_->end()) {
        return true;
      }else {
        return false;
...
class RandomAccessTableReaderScriptImpl:
 ...
 public:
  ...
  virtual bool Open(map_ss *rs, const std::string& range) { return false; }
  virtual bool Open(const std::string &rspecifier) {
    switch (state_) {
...
template<class Holder>
RandomAccessTableReader<Holder>::RandomAccessTableReader(map_ss* rs,const std::string &range):
    impl_(NULL) {
  if (rs != NULL && !Open(rs,range))
    KALDI_ERR << "Error opening RandomAccessTableReader object ";
}

template<class Holder>
bool RandomAccessTableReader<Holder>::Open(map_ss* rs,const std::string &range) {
  if(IsOpen())
    KALDI_ERR<<"Already open";
  impl_ = new RandomAccessTableReaderBufferImpl<Holder>();
  if(!impl_->Open(rs,range)){
    delete impl_;
    impl_ = NULL;
    return false; 
  }
